 ...
 */
public abstract class BaseDaoObjectifyImpl<T extends BaseModel> implements BaseDao<T> {
    ...
    private T get(Class<T> clazz, String id, boolean retry) throws EntityNotFoundException {
        T result = null;
        try {
     ...
     */
    @Override
    public LoadResult<T> getAsync(Class<T> clazz, String id) throws EntityNotFoundException {
    ...
    public Map<String, T> get(Class<T> paramClass, Iterable<String> ids) {
        Map<String, T> result = OfyService.ofy().load().type(paramClass).ids(ids);
        return result;
    }
     ...
     */
    @Override
    public Map<Long, T> getsInChunks(Class<T> paramClass, List<Long> keys, int chunkSize) {
    ...
    @Override
    public PagingResult<T> getPageOfAll(Class<T> clazz, Integer pageSize, String cursor) {
        return getPagingResult(OfyService.ofy().load().type(clazz), pageSize, cursor);
    }

    protected PagingResult<T> getPagingResult(Query<T> query, Integer pageSize, String cursor) {
    ...
    @Override
    public PagingResult<String> getPageOfKeys(Class<T> clazz, Integer pageSize, String cursor) {
        return getPagingResultKeys(OfyService.ofy().load().type(clazz), pageSize, cursor);
    }

    protected PagingResult<String> getPagingResultKeys(Query<T> query, Integer pageSize, String cursor) {
        query = query.limit(pageSize);
        if (StringUtils.isNotEmpty(cursor)) {
            query = query.startAt(Cursor.fromWebSafeString(cursor));
        }
        ...
        PagingResult<String> pagingResult = new PagingResult<String>(newCursor, list);
        return pagingResult;
    }


    public int countToday(Class<T> inclass) {
        Query query = OfyService.ofy().load().type(inclass);
        query = query.filter("createdAt >=", DateUtils.todayNoTime());
        query = query.order("createdAt");
        return getCount(inclass, query);
    }

    public int countYesterday(Class<T> inclass) {
        Query query = OfyService.ofy().load().type(inclass);
        query = query.filter("createdAt >=", DateUtils.yesterdayNoTime());
        query = query.filter("createdAt <", DateUtils.todayNoTime());
        query = query.order("createdAt");
        return getCount(inclass, query);
    }
